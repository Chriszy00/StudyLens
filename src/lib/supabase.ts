...
// caches the access token. The Web Lock timeout is irrelevant; the raw
// HTTP request is what hangs.
//
...
/**
 * Deduplicated session refresh — only ONE getSession() call runs at a time;
 * concurrent callers await the same promise. Returns the session on success,
 * null on failure.
 */
...
// ---------------------------------------------------------------------------
// Auth error detection helper — used by main.tsx's global error handler
// and by services that need to detect stale-token failures.
// ---------------------------------------------------------------------------
...
// If WE call getSession() first, it acquires a Web Lock for up to 10 seconds
// (during token refresh), blocking ALL other Supabase requests.
//
